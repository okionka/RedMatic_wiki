...
| [node-red-contrib-alexa-home-skill](https://flows.nodered.org/node/node-red-contrib-alexa-home-skill) | | ✅| ✅ | needs account| 
...
| [node-red-contrib-homekit-bridged](https://github.com/NRCHKB/node-red-contrib-homekit-bridged) | Alternative Bridge-Lösung zur Einbindung von HomeKit Accessories mit entsprechend unterstützter Characteristic durch HAP-Server über Node-RED-Knoten zur Simulation von Apple HomeKit-Geräten. Ziel ist es, native HomeKit-Geräte so nah wie möglich zu emulieren. `node-red-contrib-homekit-bridged` ermöglicht die Erstellung von vollständig anpassbarem Zubehör für die Verwendung in Apples Home App auf iOS, Watch OS und Mac OS. Besonders geeignet, wenn man ein Mesh System mit Smarthome-Devices verschiedener Hersteller nutzt, für welches kein Homekit-zugang besteht.  | ✅ | ✅ | Achtung, es kann mehr als eine Bridge konfiguriert werden. Diese müssen auf unterschiedliche Ports verweisen und in Homematic müssen diese Ports entsprechend konfiguriert sein. Andernfalls lässt sich die Bridge nicht in der der Home-App hinzufügen. Alternativ lässt man in dem Konfig-Knoten den Port frei. `node-red-contrib-homekit-bridged` sucht sich dann einen freien Port aus dem verfügbaren Pool. Das wiederum erzwingt aber ein Abschalten der Homematic Firewall (will sicher keiner), damit die Verbindung funktioniert. |
