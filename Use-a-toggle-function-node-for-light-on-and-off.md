...
The recommendation of IBM (the creator of Node-Red) is where it is always possible to implement a function with other nodes (prefered switch and change node), to dispense with function nodes. The reason is that the function node has a small performance tax as it runs in a pseudo vm environment.
As a way out you can look at the [node-red-contrib-unsafe-function](https://flows.nodered.org/node/node-red-contrib-unsafe-function) node. This is similar in memory usage and performance to the "normal" nodes.
